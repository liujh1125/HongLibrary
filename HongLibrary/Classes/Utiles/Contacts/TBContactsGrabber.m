...
//    if ([mytbcb respondsToSelector:@selector(addNewContactsIntoCustomArray)]) {
...
- (void) addNewContactsIntoCustomArray {
    ...
    for (id record in allContacts){
        ...
        if (![allNamesFromSavedArray containsObject:fullName]) {
            ...
            NSArray *sortDescriptors = [NSArray arrayWithObject:sorter];
            [self.savedArrayOfContactsWithPhoneNumbers sortUsingDescriptors:sortDescriptors];
//            [self.delegate respondsToSelector:@selector(didFinishGrabbingContactsFromAddressBook)];
            dispatch_async(dispatch_get_main_queue(), ^{
                if (self.delegate && [self.delegate respondsToSelector:@selector(didFinishGrabbingContactsFromAddressBook)]) {
                    [self.delegate didFinishGrabbingContactsFromAddressBook];
                }
